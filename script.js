...
.then((data)=>{
  if(!data.ok){
    ...
    window.location.href = 'blog_get_all.html';
    ...
    try {
        ...
        data.forEach(post => {
            ...
            postElement.innerHTML = `
                ...
                <a href="http://127.0.0.1:5500/blog_comment.html?id=${post.id}">Write & Read a Comment _ ${post.id} </a>
                <br>
...
.then(data => {
    if (data.ok) {
        ...
        window.location.href = `http://127.0.0.1:5500/blog_comment.html?id=${postId}`;
